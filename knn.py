...
def main():
    ...
    for k in [1,3,5,7,9,99,999,8000]:
      ...
      #######################################
      predicting = predict(train_X, train_y, train_X, k)
      # train_acc = 0

      # computes training accuracy from training data and predicted data
      # not true_y......
      train_acc = compute_accuracy(train_y, predicting)
      ...
      # performes the 4-fold cross validation accuracy (that is why there is a 4)
...
def get_nearest_neighbors(example_set, query, k):
    # todo
    # first section to edit!!!

    # distance between neighbores
    # Matrix/vector norm, function linalg.norm returns 1 of 8 diffrent matrix norms, or infinite
    # number of vector norms. Takes in array (has to be 1-d or 2-d), ord (order of the norm), and axis (specifies axis of
    # array to compute vector norm)
    dists = np.linalg.norm(example_set - query, ord=2, axis=1)

    # check the length of numbers of neighbors to return
    if k > (len(example_set)-1):
        k = len(example_set)-1

    k_nearest_neighbor = np.argsort(dists)
        
    #idx_of_nearest = np.sort(np.argpartition(dists, k)[:k])
    idx_of_nearest = k_nearest_neighbor[:k]
...
def knn_classify_point(examples_X, examples_y, query, k):
    ...
    nearest_neighbor = get_nearest_neighbors(examples_X, query, k)
    #print("HI")
    #print(nearest_neighbor)

    # append the nearest neighbore into nearest_neighbor_class list
    for n in range(len(nearest_neighbor)):
        nearest_neighbor_class.append(examples_y[nearest_neighbor[n]][-1])
    
    # bincount - cunts the number of occurrences of each value in an array of
    # non-negative ints. The parameters it takes in is the input array
    # input array is the list of neares neighbor.
    # predicted_label = np.bincount(nearest_neighbor_class).argmax()
    predicted_label = mode(nearest_neighbor_class)
...
def cross_validation(train_X, train_y, num_folds=4, k=1):
    # almost forgot to do this function... oops

    exData = []
    exLabels = []
    allAcc = []

    # divide train_X into num_folds sub arrays
    example_sub_arrays = np.split(train_X, num_folds)

    # divide train_y into num_folds sub arrays
    example_label_sub_arrays = np.split(train_y, num_folds)

    # for i in range of 4
    for i in range(num_folds):
        # cast example_sub_arrays as a list to compare
        tData = example_sub_arrays[i].tolist()
        tLabels = example_label_sub_arrays[i].tolist()

        # put all matricies into one stack
        exLabels = np.vstack([l for j,l in enumerate(example_label_sub_arrays) if j != i])
        exData = np.vstack([l for j,l in enumerate(example_sub_arrays) if j != i])

        # handel edge case of k which is 8000 it's not possible to 
        # compare 8000 nearest neighbors with 6000 total points
        k = min(k, len(exData))
        predicted_labels = predict(exData, exLabels, tData, k)
        accuracy = compute_accuracy(tLabels, predicted_labels)

        # append current accuracy into the list where all the accuracies are held
        allAcc.append(accuracy)
    
    # average validation accuracy accross 4 folds
    avg_val_acc = np.mean(allAcc)

    # variance of validation accuracy accross 4 folds
    varr_val_acc = np.var(allAcc)
